...
NEW_SYS_MSG_LINES = [
    ...
    '            "3) 保留数字/日期/专有名词；如无常见译名可保留原文。不要为了顺口而改变含义。"',
    '            "4) 每条输出一行，严格保持条目数量一致。输出格式必须为："',
    '            "N) <TITLE的中文翻译> ||| <SNIP的中文翻译> 。SNIP 可为空但分隔符必须保留。"',
    '            "5) 除了上述格式，不得输出任何多余内容（不许编号外加标题、不许多段、不许注释）。"',
    '        )',
]
...
def main():
    ...
    print("OK: tightened translation sys_msg prompt v1.2 (field-locked, no inference)")
