...
def _insert_helpers(src):
    ...
    block = "\n" + HELPER_BEGIN + "\n" + (
        ...
        "        # whole next month (only for calendar-like queries)\n"
        ...
        "            # bare 周三: if already passed this week, move to next week\n"
        "            if prefix == \"周\":\n"
...
def _inject_weather(src):
    # insert right after t2 normalization line
    m = re.search(r"^def\s+_weather_range_from_text\s*\(.*?\):\s*$", src, flags=re.M)
    if not m:
        raise RuntimeError("Cannot find _weather_range_from_text")
    sub = src[m.start():]
    m2 = re.search(r"^\s*t2\s*=\s*re\.sub\([^\n]+\)\s*$", sub, flags=re.M)
    if not m2:
        raise RuntimeError("Cannot find t2 normalization inside _weather_range_from_text")
    anchor_end = m.start() + m2.end()

    # avoid double insert
    if "CN_RANGE_EXT_V1 APPLY WEATHER" in sub[:m2.end()+300]:
        return src, False
...
def _inject_calendar(src):
    m = re.search(r"^def\s+_calendar_range_from_text\s*\(.*?\):\s*$", src, flags=re.M)
    if not m:
        raise RuntimeError("Cannot find _calendar_range_from_text")
    sub = src[m.start():]
    m2 = re.search(r"^\s*t2\s*=\s*re\.sub\([^\n]+\)\s*$", sub, flags=re.M)
    if not m2:
    ...
    if "CN_RANGE_EXT_V1 APPLY CALENDAR" in sub[:m2.end()+300]:
        return src, False
...
def main():
    ...
    with io.open(APP + ".bak.cn_range_ext_v1c", "w", encoding="utf-8") as f:
        f.write(src)

    _write(src4)
    print("Patched OK. Backup: " + APP + ".bak.cn_range_ext_v1c")
