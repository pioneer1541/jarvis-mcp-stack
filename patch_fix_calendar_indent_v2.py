...
# 3) 找 calendar 之后同缩进层级的“下一个 structured_* 分支”作为结束锚点
rest = fn[m_cal.end():]
m_next_branch = re.search(
    r"(?m)^" + re.escape(indent) + r"(if|elif)[ \t]+rt[ \t]*==[ \t]*['\"]structured_[a-z0-9_]+['\"][ \t]*:[ \t]*$",
    rest,
)
if not m_next_branch:
    # 再兜底：如果找不到下一个 structured 分支，就找 open_domain 的兜底 return
    m_open = re.search(r'(?m)^' + re.escape(indent) + r'return[ \t]+\{[ \t]*"ok"[ \t]*:[ \t]*True[ \t]*,[ \t]*"route_type"[ \t]*:[ \t]*"open_domain"', rest)
    if not m_open:
        raise RuntimeError("cannot find next structured_* branch after structured_calendar (and no open_domain fallback found)")
    cal_end = m_cal.end() + m_open.start()
else:
    cal_end = m_cal.end() + m_next_branch.start()

# 4) 构造正确缩进的 structured_calendar 分支（包含排序 + 口播摘要 + 必 return）
lines = []
