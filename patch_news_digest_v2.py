...
def _news__rank_zh_first(items: list) -> list:
    def keyfn(it):
        ...
        # zh first (False sorts after True if we invert)
...
def news_digest(category: str = "world", time_range: str = "day", limit: int = 5) -> dict:
    """
    Semi-structured news digest. zh-first, fallback to en pools; if still empty, widen time_range to week.
    Returns:
      {ok, category, time_range, limit, final, items, query_used}
    """
    ...
    # 3) still empty and tr==day => widen to week once
...
def main():
    ...
    # 1) Insert NEWS_BLOCK after def web_search(...) ends, before @mcp.tool(description="(Tool) Open an URL"
    ...
    # 2) Patch _route_type to return semi_structured_news when _is_news_query
    # Insert just before: return open_domain
    ...
    # make sure we insert only once near the first occurrence inside _route_type
    # Use a safer anchor: find the _route_type function block start and its 'return open_domain'
    ...
    if "semi_structured_news" not in fn_slice:
        ...
        # insert right before the 'return open_domain' within _route_type
        s2 = s2[:rt_pos] + insert_snippet + s2[rt_pos:]

    # 3) Patch route_request to handle semi_structured_news
    rr_start = s2.find("def route_request(")
    if rr_start < 0:
        print("ERROR: cannot find def route_request")
        sys.exit(6)

    # Find place after: route_type = _route_type(user_text)
    ...
    # insert after the assignment line end
    line_end = s2.find("\n", kpos)
    if line_end < 0:
    ...
    print("OK: patched app.py with news digest v2")
